...
void Tile::render(GameContainer* container, Renderer* r)
{
	...
	bool isLeftEdge = (gx == 0 || (gx > 0 && !igs->getTileAt(gx-1, gy)->solid));
	bool isRightEdge = (gx == igs->worldWidth-1 || ((gx < igs->worldWidth-1 && !igs->getTileAt(gx+1, gy)->solid)));
	bool isTopEdge = (gy == 0 || (gy > 0 && !igs->getTileAt(gx, gy-1)->solid));
	bool isBottomEdge = (gy == igs->worldHeight-1 || (gy <= igs->worldHeight-1 && !igs->getTileAt(gx,gy+1)->solid));
